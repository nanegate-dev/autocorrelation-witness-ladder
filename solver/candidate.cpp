...
int main(int argc, char** argv) {
    ...
    constexpr int kRestarts = 24;  // evolve:param restarts int 1 64
    ...
    constexpr int kSteps = 60000;  // evolve:param steps int 200 100000
    // Step size, before the 1/(1 + decay*t) taper below.
    constexpr double kRate = 0.005;  // evolve:param rate real 0.0001 1.0
    ...
    constexpr double kMomentum = 0.999;  // evolve:param momentum real 0.0 0.99999
    // Where the anneal starts. Low is smooth and leads somewhere.
    constexpr double kBetaFrom = 200.0;  // evolve:param beta_from real 1.0 100000.0
    ...
    constexpr double kBetaTo = 2000000.0;  // evolve:param beta_to real 1000.0 100000000.0
    ...
    for (int restart = 0; restart < kRestarts; ++restart) {
        ...
        if (restart == 0) {
            ...
            double mass = 0.0;
            for (int i = 0; i < n; ++i) { h[i] = 0.2 + 1.8 * rng.unit(); mass += h[i]; }
        ...
        for (int step = 0; step < kSteps; ++step) {
            ...
            const double rate = kRate / (1.0 + 2e-5 * static_cast<double>(step));
            for (int i = 0; i < n; ++i) {
